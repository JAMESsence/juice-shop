...
router.post('/', async (req: Request<Record<string, unknown>, Record<string, unknown>, DataErasureRequestParams>, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    const viewModel: Pick<DataErasureRequestParams, 'email' | 'securityAnswer'> = {
    ...
    if (req.body.layout) {
      ...
      if (!isForbiddenFile) {
        res.render('dataErasureResult', viewModel, (error, html) => {
          if (!html || error) {
      ...
      res.render('dataErasureResult', viewModel)
    }
  } catch (error) {
    next(error)
  }
